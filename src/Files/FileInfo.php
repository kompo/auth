...
class FileInfo extends Form
{
    ...
    {
        return _CardWhite(
            _Html(__('ka::files.file-name', ['name' => $this->model->name]))->class('text-sm font-bold mb-1'),
            _Html(__('ka::files.file-size', ['size' => $this->model->size]))->class('text-sm'),
            _Flex(
                _Html(__('ka::files.file-uploaded-at'))->class('text-sm'),
                $this->model->uploadedAt()->class('!text-sm !text-black'),
            )->class('gap-2'),
            $this->fileableInfo(),
            _Button('ka::files.edit-form')
    ...
    protected function fileableInfo()
    {
        ...
        return _Rows(
            _Html(__('ka::files.fileable-type', ['type' => $this->model->fileable_type]))->class('text-sm'),
            _Html(__('ka::files.fileable-id', ['id' => $this->model->fileable->id]))->class('text-sm'),
        );
    }
}
