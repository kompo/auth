	...
	public function top()
	{
		return _Rows(
			_FlexBetween(
				...
				_Title('ka::files.attach-from-library')->class('text-2xl sm:text-3xl font-bold')
					->icon('document-text')
					->class('font-semibold mb-4 md:mb-0')
					->class('flex items-center'),

				_FlexEnd(
					_Button('ka::files.confirm')->getElements('confirmSelection')->inPanel('linked-attachments')
						->closeModal()
	...
	public static function selectedFiles($selectedIds = [])
	{
		...
		return _Rows(
			!$selectedFiles->count() ? null : _MultiSelect()->name('selected_files', false)
				...
				->class('mb-0'),
			_Button('ka::files.add-from-library')
