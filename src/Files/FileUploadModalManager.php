...
class FileUploadModalManager extends Modal
{
	...
	protected $_Title = 'ka::files.upload-one-multiple-files';
	...
	{
		return _Rows(
            $this->model->id ? null : _MultiFile()->name('files')->placeholder('ka::files.upload-one-multiple-files')
                ->class('text-gray-600 large-file-upload mb-10'),
            _Rows(
                _TagsMultiSelect()->class('mb-10'),
            ),
            _Rows(
                _Html('ka::files.fileable')->class('text-lg font-semibold mb-2'),
