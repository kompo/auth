...
class Notification extends Model implements NoTeamScope, HasOwnedRecords
{
    ...
    public function reminderDropdown()
    {
        return _Dropdown('notifications-button-remind-me-again')->rIcon('icon-down')
            ...
            ->dropdownOverModal(sumWidth: true, heightAdd: 20)
            ->alignRight();
    }
    ...
    public function genericNotificationCard($title, $buttons, $hasReminderButton = false)
    {
        ...
        $buttonsBlock = empty($buttons) ? null : _Flex(
            ...collect($buttons)->map(fn ($b) => $b->class('flex-1')->when(count($buttons) === 1, fn ($b) => $b->class('xl:flex-[unset]')))->all(),
        )->class('gap-2 flex-wrap flex-1')->when(count($buttons) === 1, fn ($b) => $b->class('xl:!justify-end'));
        ...
        return _Rows(
            ...
            )->class('gap-4 flex-wrap xl:!justify-end')
        );
    }
